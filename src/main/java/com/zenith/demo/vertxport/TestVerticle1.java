...
public class TestVerticle1  extends AbstractVerticle {
    ...
    public void start(Future<Void> startFuture) throws Exception {
        ...
        // 处理一个post方法的rest接口
        router.post("/post1/:param1/:param2").handler(this::handlePost);
        // 处理一个get方法的rest接口
        router.get("/get1/:param1/:param2").handler(this::handleGet);
        // 创建一个httpserver，监听8080端口，并交由路由器分发处理用户请求
        vertx.createHttpServer()
        .requestHandler(router::accept)
        .listen(8080, ar -> {
          if (ar.succeeded()) {
             System.out.println("HTTP server1 running on port " + 8080);
